...
class LineageVisualization:
        ...
        try:
            ...
            if root_cell_id is None and len(top_components) > 1:
                for i, component in enumerate(top_components):
                    ...
                    if not roots:
                        axes[i].text(0.5, 0.5, "No root node", ha="center", va="center")
                        axes[i].axis("off")
                        continue

                    pos = self.hierarchy_pos(subgraph, roots[0])
                    node_sizes = [
                        100 + subgraph.nodes[n]["duration"] * 10
                        for n in subgraph.nodes()
                    ]
                    node_colors = [
                        "red" if subgraph.nodes[n]["divides"] else "blue"
                        for n in subgraph.nodes()
                    ]
                    ...
                    nx.draw_networkx_labels(subgraph, pos, font_size=8, ax=axes[i])
                ...
                if not roots:
                    axes[0].text(0.5, 0.5, "No root node", ha="center", va="center")
                    axes[0].axis("off")
                else:
                    pos = self.hierarchy_pos(G, roots[0])
                    node_sizes = [100 + G.nodes[n]["duration"] * 10 for n in G.nodes()]
                    node_colors = [
                        "red" if G.nodes[n]["divides"] else "blue" for n in G.nodes()
                    ]
                    ...
                    title = (
                        f"Lineage Tree for Cell {root_cell_id}"
                        if root_cell_id
                        else "Largest Lineage Tree"
                    )
    ...
    def hierarchy_pos(self, G, root, width=1.0, vert_gap=0.1, vert_loc=0, xcenter=0.5):
        """Hierarchical layout where each subtree gets horizontal space
        proportional to its number of leaves. Prevents sibling collapse."""
        import networkx as nx

        def leaf_count(node, visited):
            visited.add(node)
            kids = [c for c in G.successors(node) if c not in visited]
            if not kids:
                return 1
            return sum(leaf_count(c, visited) for c in kids)

        def _recurse(node, left, right, depth, pos, visited):
            visited.add(node)
            pos[node] = ((left + right) / 2, vert_loc - depth * vert_gap)
            kids = [c for c in G.successors(node) if c not in visited]
            if not kids:
                return
            total_leaves = sum(leaf_count(c, set()) for c in kids)
            span = right - left
            cursor = left
            for c in kids:
                w = span * (leaf_count(c, set()) / total_leaves)
                _recurse(c, cursor, cursor + w, depth + 1, pos, visited)
                cursor += w

        pos = {}
        _recurse(root, xcenter - width / 2, xcenter + width / 2, 0, pos, set())
        return pos
    ...
    def visualize_morphology_lineage_tree(self, tracks, canvas, root_cell_id=None):
        ...
        try:
            ...
            try:
                ...
                for level, nodes in nodes_by_level.items():
                    y = 0.9 - (
                        level / max(1.0, float(max_level)) * 0.8
        ...
        self.node_states = {
            node: {"frame_appeared": -1, "animation_phase": 0} for node in G.nodes()
        }
        ...
        def update(frame):
            ...
            for node_id in nodes_to_show:
                ...
                self.node_states[node_id]["animation_phase"] = (
                    frame - self.node_states[node_id]["frame_appeared"]
                )
            ...
            for parent, child in G.edges():
                if parent in nodes_to_show and child in nodes_to_show:
                    ...
                    delay = (
                        5 if is_second_edge else 0
                    )  # Delay the second line by 5 frames
                    ...
                    if self.edge_states[(parent, child)]["frame_appeared"] == -1:
                        ...
                        if is_different_level:
                            ...
                            if parent_frame >= 0:
                                self.edge_states[(parent, child)]["frame_appeared"] = (
                                    parent_frame + 10 + delay
                                )
                        else:
            ...
            for node_id in nodes_to_show:
                if (
                    self.node_states[node_id]["frame_appeared"] >= 0
                    and frame >= self.node_states[node_id]["frame_appeared"]
                ):
                    x, y = pos[node_id]
                    ...
                    try:
                        ...
                        if roots:
                            path_length = (
                                len(nx.shortest_path(G, roots[0], node_id)) - 1
                            )
                        else:
                            path_length = 0
                    except:
    ...
    def collect_cell_morphology_data(self, tracks):
        ...
        if hasattr(self, "cell_mapping") and self.cell_mapping:
            ...
            for track in tracks:
                ...
                for cell_id, cell_data in self.cell_mapping.items():
                    if (
                        "metrics" in cell_data
                        and "morphology_class" in cell_data["metrics"]
                    ):
                ...
                if track_id not in morphology_data:
                    if divides:
                        ...
                        if "x" in track and "y" in track and len(track["x"]) > 3:
                            ...
                            straightness = (
                                direct_distance / path_length if path_length > 0 else 1
                            )
            ...
            for track in tracks:
                ...
                if divides:
                    ...
                    for other_track in tracks:
                        if (
                            "children" in other_track
                            and track_id in other_track["children"]
                        ):
                            has_parent = True
                            break
        ...
        for morph_class in required_morphologies:
            if (
                morph_class not in morphology_counts
                or morphology_counts[morph_class] == 0
            ):
                ...
                for track_id, current_class in morphology_data.items():
                    ...
                    if current_class in morphology_counts and morphology_counts[
                        current_class
                    ] > max(2, len(morphology_data) // 10):
                ...
                if candidates:
                    ...
                    for i in range(convert_count):
                        ...
                        # Update counts
                        morphology_counts[old_class] = (
                            morphology_counts.get(old_class, 0) - 1
                        )
                        morphology_counts[morph_class] = (
                            morphology_counts.get(morph_class, 0) + 1
                        )
    ...
    def draw_morphology_nodes(self, G, pos, ax, full_graph, update=False):
        ...
        legend_elements = [
            ...
            plt.Line2D(
                ...
                marker="o",
                color="w",
                markerfacecolor=colors["Elongated"],
                label="Elongated Cell (stretching)",
                markersize=10,
            ),
            plt.Line2D(
                [0],
                ...
                marker="o",
                color="w",
                markerfacecolor=colors["Deformed"],
                label="Deformed Cell (wobbling)",
                markersize=10,
            ),
            plt.Line2D(
                [0],
                ...
                marker="o",
                color="w",
                markerfacecolor=colors["Artifact"],
                label="Artifact",
                markersize=10,
            ),
        ]
    ...
    def visualize_focused_lineage_tree(
        self, root_cell_id, output_path=None, progress_callback=None
    ):
        """
        ...
        node_colors = [
            "skyblue" if node != root_cell_id else "red" for node in G.nodes()
        ]
        ...
        plt.figtext(
            0.02, 0.02, info_text, bbox=dict(facecolor="white", alpha=0.8), fontsize=10
        )
    ...
    def create_cartoony_lineage_comparison(
        self, tracks, canvas, root_cell_id=None, time_point="last"
    ):
        """
        ...
        legend = ax.legend(
            ...
            loc="upper right",
            fontsize=8,
            title=f"Morphology at {time_point} time",
            title_fontsize=9,
            frameon=True,
            facecolor="#444444",
            edgecolor="gray",
    ...
    def draw_timepoint_nodes(self, G, pos, ax, time_point):
        ...
        for morphology, nodes in morphology_groups.items():
            ...
            color = self.morphology_colors_rgb.get(
                morphology, (0.5, 0.5, 0.5)
            )  # Default to gray

            # Node size based on whether the cell divides
            node_sizes = [
                300 if G.nodes[n].get("has_children", False) else 150 for n in nodes
            ]
    ...
    def calculate_diversity_metrics(self, tracks):
        ...
        # Calculate metrics
        internal_diversity = (
            internal_changes / total_cells_with_internal_data
            if total_cells_with_internal_data > 0
            else 0
        )
        robustness = (
            generational_matches / total_parent_child_pairs
    ...
    def get_morphology_at_time(self, track, time):
        ...
        if (
            hasattr(self, "morphology_by_time")
            and track_id in self.morphology_by_time
            and time in self.morphology_by_time[track_id]
        ):
        ...
        if hasattr(self, "cell_mapping") and self.cell_mapping:
            ...
            for cid, cell_data in self.cell_mapping.items():
                if (
                    "track_id" in cell_data
                    and cell_data["track_id"] == track_id
                    and "time" in cell_data
                    and cell_data["time"] == time
                ):
                    cell_id = cid
                    break
            ...
            if (
                cell_id
                and cell_id in self.cell_mapping
                and "metrics" in self.cell_mapping[cell_id]
            ):
    ...
    def export_morphology_classifications(self, parent_widget=None):
        ...
        save_path, _ = QFileDialog.getSaveFileName(
            parent_widget,
            "Save Morphology Classification Data",
            "",
            "CSV Files (*.csv)",
        )
        ...
        try:
            ...
            for track_id, time_dict in self.morphology_by_time.items():
                for time, morphology in time_dict.items():
                    rows.append(
                        {"track_id": track_id, "time": time, "morphology": morphology}
                    )
    ...
    def visualize_growth_and_division(self, tracks, growth_metrics):
        ...
        axes[0, 0].axvline(
            growth_metrics["avg_division_time"],
            color="red",
            linestyle="--",
            label=f"Mean: {growth_metrics['avg_division_time']:.1f}",
        )
        axes[0, 0].axvline(
            growth_metrics["median_division_time"],
            color="green",
            linestyle="--",
            label=f"Median: {growth_metrics['median_division_time']:.1f}",
        )
        axes[0, 0].legend()

        # 2. Histogram of growth rates
        axes[0, 1].hist(
            growth_metrics["growth_rates"],
            bins=20,
            color="lightgreen",
            edgecolor="black",
        )
        ...
        axes[0, 1].axvline(
            growth_metrics["avg_growth_rate"],
            color="red",
            linestyle="--",
            label=f"Mean: {growth_metrics['avg_growth_rate']:.4f}",
        )
        axes[0, 1].legend()
        ...
        for track in tracks:
            if (
                "children" in track
                and track["children"]
                and track["ID"] in division_time_by_id
            ):
