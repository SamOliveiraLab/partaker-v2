...
def update(frame):
    ...
    for node_id in nodes_to_show:
        ...
        node_states[node_id]['animation_phase'] = frame - node_states[node_id]['frame_appeared']
    ...
    for node_id in nodes_to_show:
        ...
        bounce = 1 + 0.2 * np.sin(np.pi * min(phase / 10, 1))  # Slower bounce over 10 frames
...
try:
    ...
    ani.save('ecoli_lineage_tree.mp4', writer='ffmpeg', fps=5)  # Slower playback (5 fps)
...
try:
    ...
    ani.save('ecoli_lineage_tree.gif', writer='pillow', fps=5)  # Slower playback (5 fps)
